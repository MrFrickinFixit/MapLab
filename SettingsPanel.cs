...
public sealed class SettingsPanel : Grid
{
    ...
    public SettingsPanel(Action openFile, Action saveFile, Action saveFileAs, Action openColors)
    {
        ...
        content.Children.Add(new Border { Height = 1, Background = new SolidColorBrush(Color.FromRgb(225, 225, 225)), Margin = new Thickness(0, 18, 0, 14) });
        content.Children.Add(new TextBlock { Text = "3D NAVIGATION", Foreground = new SolidColorBrush(Color.FromRgb(94, 94, 94)), FontSize = 11, FontWeight = FontWeights.SemiBold, Margin = new Thickness(0, 0, 0, 8) });
        content.Children.Add(new TextBlock { Text = "Choose the input profile used by all Timing, Fueling, and Sandbox 3D viewers. The viewer also provides Select, Orbit, and Pan modes so every action is available with either device.", TextWrapping = TextWrapping.Wrap, Foreground = new SolidColorBrush(Color.FromRgb(32, 32, 32)), FontSize = 13, LineHeight = 20, Margin = new Thickness(0, 0, 0, 10) });
        var navigationProfile = new ComboBox { Width = 180, Height = 29, HorizontalAlignment = HorizontalAlignment.Left, Background = Brushes.White, Foreground = Brushes.Black, Padding = new Thickness(6, 2, 6, 2) };
        navigationProfile.Items.Add("Desktop mouse"); navigationProfile.Items.Add("Laptop touchpad");
        navigationProfile.SelectedIndex = Surface3DNavigationPreferences.Current == Surface3DInputProfile.DesktopMouse ? 0 : 1;
        navigationProfile.SelectionChanged += (_, _) => Surface3DNavigationPreferences.Current = navigationProfile.SelectedIndex == 1 ? Surface3DInputProfile.LaptopTouchpad : Surface3DInputProfile.DesktopMouse;
        Surface3DNavigationPreferences.Changed += profile => Dispatcher.Invoke(() =>
        {
            var selectedIndex = profile == Surface3DInputProfile.DesktopMouse ? 0 : 1;
            if (navigationProfile.SelectedIndex != selectedIndex) navigationProfile.SelectedIndex = selectedIndex;
        });
        content.Children.Add(navigationProfile);
