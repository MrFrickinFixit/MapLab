...
public sealed class OffsetSelectionWindow : Window
{
    ...
    private Action<int, double, bool> applyOffset;
    ...
    public OffsetSelectionWindow(double amount, bool isPercentage, Action<int, double, bool> applyOffset)
    {
        ...
        var unitPanel = new StackPanel { Margin = new Thickness(8, 0, 0, 0) }; unitPanel.Children.Add(Label("OFFSET UNIT")); unitBox = new ComboBox { Background = Brushes.White, Foreground = Brushes.Black, Padding = new Thickness(8, 5, 8, 5) }; unitBox.Items.Add(new ComboBoxItem { Content = "Numerical", Foreground = Brushes.Black }); unitBox.Items.Add(new ComboBoxItem { Content = "Percentage of value", Foreground = Brushes.Black }); unitBox.SelectedIndex = isPercentage ? 1 : 0; unitPanel.Children.Add(unitBox); Grid.SetColumn(unitPanel, 1); settings.Children.Add(unitPanel); Grid.SetRow(settings, 1); root.Children.Add(settings);
    ...
    public void Configure(double amount, bool isPercentage, Action<int, double, bool> callback)
    {
        applyOffset = callback; Amount = amount; IsPercentage = isPercentage;
        amountBox.Text = amount.ToString("0.###", CultureInfo.InvariantCulture);
        unitBox.SelectedIndex = isPercentage ? 1 : 0;
    }
