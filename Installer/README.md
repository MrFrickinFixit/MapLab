...
```text
artifacts\installer\MapLab-1.0.3.7-beta-win-x64.msi
```
...
.\scripts\build-inno-installer.ps1 -Version 1.0.3.7 -CompilerPath 'C:\Program Files (x86)\Inno Setup 6\ISCC.exe'
```
...
defaults to version `1.0.3.7`. The installed application version is displayed as
`1.0.3.7-beta` while Windows file-version metadata remains numeric.
